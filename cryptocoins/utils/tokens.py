...
from core.consts.currencies import TRRXITTE_CURRENCIES  # Your custom ETX20 tokens dict
...
def get_token_contract_address(token_currency_code: str, blockchain_currency_code: str):
    ...
    blockchain_tokens_dict: Dict[str, Dict[Currency, TokenParams]] = {
        'ETH': ERC20_CURRENCIES,
        'TRX': TRC20_CURRENCIES,
        'BNB': BEP20_CURRENCIES,
        'TRRXITTE': TRRXITTE_CURRENCIES,  # Add ETX20 tokens here
    }

    if blockchain_currency_code not in blockchain_tokens_dict:
        raise Exception(f'Blockchain currency {blockchain_currency_code} is not registered')

    token_params = blockchain_tokens_dict[blockchain_currency_code].get(Currency.get(token_currency_code))
    if not token_params:
        raise Exception(f'Contract address not found for {token_currency_code} on blockchain {blockchain_currency_code}')
